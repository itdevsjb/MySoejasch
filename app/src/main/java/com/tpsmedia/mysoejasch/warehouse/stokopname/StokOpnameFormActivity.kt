...
import com.tpsmedia.mysoejasch.databinding.ActivityStokopnameFormBinding
...
class StokOpnameFormActivity : AppCompatActivity() {
    private lateinit var binding: ActivityStokopnameFormBinding
    private var drawerLayout: DrawerLayout? = null


    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityStokopnameFormBinding.inflate(layoutInflater)
                        ...
                        binding.spinnerUCodeLokasi.onItemSelectedListener =
                            object : AdapterView.OnItemSelectedListener {
                                override fun onItemSelected(
                                    ...
                                    fetchSpinnerDataPallet(selectedItem.value)
                                }
                                override fun onNothingSelected(parentView: AdapterView<*>?) {
                                }
                            }
    ...
    private fun fetchSpinnerDataPallet(ucode_lokasi: String) {
        val serviceLogin = ServiceLogin(this)
        val service2 = Client.getClient(this).create(
            Interface::class.java
        )
        val call = service2.postPalletArr("Bearer "+ serviceLogin.token, serviceLogin.loginId, ucode_lokasi)
        call.enqueue(object : Callback<Palletrequest> {
            override fun onResponse(call: Call<Palletrequest>, response: Response<Palletrequest>) {
                if (response.isSuccessful) {
                    response.body()?.data?.let { spinnerItems ->
                        ...
                        adapterku.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item)
                        binding.spinnerUCodePallet.adapter = adapterku
                        binding.spinnerUCodePallet.onItemSelectedListener =
                            object : AdapterView.OnItemSelectedListener {
                                override fun onItemSelected(
                                    parentView: AdapterView<*>?,
                                    selectedItemView: View?,
                                    ...
                                    val editor = sgSharedPref.edit()
                                    editor.putString("ucode_pallet", selectedItem.value)
                                    editor.apply()
                                }
                                override fun onNothingSelected(parentView: AdapterView<*>?) {
                                }
            ...
            override fun onFailure(call: Call<Palletrequest>, t: Throwable) {
                Log.e("API Error", "Request failed: ${t.message}")
            }
        })
    }
    ...
    private fun submitData() {
        ...
        jsonObject.put("ucode_pallet", serviceData.ucode_pallet)
        ...
        val call = service2.postHeadOpnameBaru("Bearer "+ serviceLogin.token, requestBody)
