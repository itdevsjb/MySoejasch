...
import com.tpsmedia.appmanager.model.Category
...
class ServerManager {
    ...
    fun getPosts(context: Context, function: (posts: ArrayList<PostModel>?) -> (Unit)) {
        ...
        if (localPost.isEmpty()) {
            getPostResponse(context, "http://192.168.0.206/api-portal-soejasch/public/api/cabang") {
                Log.d("LOG", "getPosts :  " + it?.datareal?.size)
                function(it?.datareal)
            }
        } else {
            function(localPost)
        }
    ...
    fun getPrOnlines(context: Context, function: (posts: ArrayList<PROnlineModel>?) -> (Unit)) {
        val localPost = LocalManager().getPrOnlines(context)
        if (localPost.isEmpty()) {
            getPostResponse(context, "http://192.168.0.206/api-portal-soejasch/public/api/cabang") {
